...
def test_extract_report_url_from_merged_pr_body() -> None:
    """A completed Issue with a merged PR whose body carries the template
    field `**报告**: <url>` gets the report URL."""
    with patch.object(MOD.subprocess, "run") as mocked:
        mocked.side_effect = [
            SimpleNamespace(returncode=0, stdout=json.dumps([
                {"number": 162, "state": "MERGED",
                 "body": "Resolves #145.\n**报告**: https://github.com/example-org/sample/blob/main/results/bj145_gate_b_summary.md"},
            ]), stderr=""),
        ]
