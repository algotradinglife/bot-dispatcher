...
def test_human_escape_notifies_max_3(tmp_path):
    """Human 持续 8h+ → 每小时 1 次, 上限 3 次 (计数锁内自增, 无需外部确认)."""
...
def test_human_escape_1h_window_no_resend(tmp_path):
    """1h 窗口内不重发 (计数自增 + last 时间戳双重限频)."""
